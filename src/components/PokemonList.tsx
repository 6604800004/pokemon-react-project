...
function PokemonList() {
    ...
    const filterData = useMemo(() =>
        data.filter((pokemon) =>
            pokemon.id.padStart(4, '0').includes(inputText) ||
            pokemon.name.toLowerCase().includes(inputText.toLowerCase()) ||
            pokemon.types.some((type) => type.toLowerCase().includes(inputText.toLowerCase()))
    ...
    const fetchSpecies = async (currentOffset: number) => {
        setLoading(true);
        const res = await fetch(`https://pokeapi.co/api/v2/pokemon-species?limit=20&offset=${currentOffset}`);
            ...
            speciesListData.results.map(async (species: { url: string }) => {
                ...
                return Promise.all(
                    sData.varieties
                        ...
                        .filter((v: { pokemon: { name: string } }) => !v.pokemon.name.includes('raichu-mega'))
                        .map(async (v: { pokemon: { name: string; url: string } }) => {
                            ...
                            return {
                                ...
                                types: pData.types.map((t: { type: { name: string } }) => t.type.name)
                            };
                        })
                );
            })
        ...
        <>
            ...
            <div className="search-container">
                <div className="pokemon-list__search">
                    <div className="pokemon-search_from__text">ค้นหาด้วยชื่อ หรือ หมายเลขโปเกเด็กซ์</div>
                    <input
                        type="text"
                    ...
                    {filterData.map((pokemon) => (
                        ...
                        >
                            <img
                                ...
                                alt={pokemon.name}
                            />
                            <div className="pokemon-list--box__no">{pokemon.id.padStart(4, '0')}</div>
                            <div className="pokemon-list--box__name">{pokemon.name}</div>
                            <div className="pokemon-list--box__types__size">
                                <div className="pokemon-list--box__types">{pokemon.types.map((type) => (
                ...
                <div className="pokemon-list-load-more__button">
                    ...
                    ) : hasMore && (!inputText || filterData.length > 0) && (
